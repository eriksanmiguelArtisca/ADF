...
export class CustomDialogsService {
    ...
    openFileBrowseDialogByFolderId(folderNodeId: string): Observable<Node[]> {
        return this.documentListService.getFolderNode(folderNodeId).pipe(
            switchMap((nodeEntry: NodeEntry) => {
    ...
    public openLockNodeDialog(contentEntry: Node): Subject<string> {
        ...
        if (
            this.contentService.hasAllowableOperations(
                contentEntry,
    ...
    openFileBrowseDialogBySite(): Observable<Node[]> {
        return this.siteService.getSites().pipe(
            switchMap((response: SitePaging) => {
                return this.openFileBrowseDialogByFolderId(
                    response.list.entries[0].entry.guid
                );
            })
        );
    }
    ...
    openFolderBrowseDialogBySite(): Observable<Node[]> {
        return this.siteService.getSites().pipe(
            switchMap((response: SitePaging) => {
                return this.openFolderBrowseDialogByFolderId(
                    response.list.entries[0].entry.guid
                );
            })
        );
    ...
    openCopyMoveDialog(
        ...
        if (
            this.contentService.hasAllowableOperations(contentEntry, permission)
        ) {
            ...
            const data: ContentNodeSelectorComponentData = {
                ...
                rowFilter: this.filtroNodos.bind(this),
                excludeSiteContent:
                    excludeSiteContent ||
                    ContentNodeDialogService.nonDocumentSiteContent,
                select: select,
                breadcrumbTransform: this.breadcrumbTransform.bind(this),
    ...
    openUploadFolderDialog(
        ...
        const data: ContentNodeSelectorComponentData = {
            ...
            imageResolver: this.imageResolver.bind(this),
            isSelectionValid: this.hasAllowableOperationsOnNodeFolder.bind(
                this
            ),
            where: '(isFolder=true)',
            rowFilter: this.filtroNodos.bind(this),
    ...
    filtroNodos(row: ShareDataRow) {
        let node = row.node.entry;
        let nombreNodo: string = node.name;

        if (nombreNodo === "Carpeta entrada inicial docs" ) {
            return false;
        } else {
            return true;
        }
    ...
    /**
     * Edit the elemenths of the path of the node so the toolbar with the path don't show certain elements
     * @param node Node of the current folder
     * @returns Node with the midified path elements
     */
    breadcrumbTransform(node: any) {
        if (node && node.path && node.path.elements) {
            const elements = node.path.elements;
            if (elements.length > 1) {
                let posCompany = elements.findIndex( (node) => node.name == 'Company Home' );
                if (posCompany !== -1){
                    elements.splice(posCompany, 1);
                }
                let posSites = elements.findIndex( (node) => node.name == 'Sites' );
                if (posSites !== -1){
                    elements.splice(posSites, 1);
                }
    ...
    getTitleTranslation(action: string, name: string): string {
        if(
            ...
            return this.translation.instant(
                `NODE_SELECTOR.${action.toUpperCase()}`,
            )
        }
        return this.translation.instant(
            `NODE_SELECTOR.${action.toUpperCase()}_ITEM`,
            { name }
        );
    }
    ...
    private hasAllowableOperationsOnNodeFolder(entry: Node): boolean {
        return (
            this.isNodeFolder(entry) &&
            this.contentService.hasAllowableOperations(entry, 'create')
        );
    }
